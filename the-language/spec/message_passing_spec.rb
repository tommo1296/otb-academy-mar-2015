RSpec.describe "message passing" do
  ...
  it "can also use __send__" do
    ...
    expect( mc.__send__(:caught?) ).to eq( true ) #so this is because send can be re specified so __send__ was made

    #additionally, id has been reused and is now object_id
  ...
  it "can send a message with arguments" do
    ...
    expect( mc.send(:add_a_payload, 3, 4, nil, 6) ).to eq( [3,4,nil,6] ) # this works, but i bet if there was multiple parameters you would need ())
